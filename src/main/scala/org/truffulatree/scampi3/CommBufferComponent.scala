...
trait CommBufferComponent {
  ...
  trait CommRegion extends CommAddressable {
    ...
    def bind[A](dv: mpi3.SeqDatatypeVector[A], at: Long = 0):
        SeqValueBuffer[A] = {
      if (root == this) {
    ...
    final def @:[A](dv: mpi3.SeqDatatypeVector[A]): SeqValueBuffer[A] =
      bind(dv, 0)

    final def @:[A](dt: mpi3.SeqDatatype[A]): SeqValueBuffer[A] = {
      val mpi3.Extent(_, range) = dt.extent
      val mpi3.Extent(_, trueRange) = dt.trueExtent
  ...
  case class CommRegionMarker(val root: CommRegion, val offset: Long)
    ...
    def @:[A](dv: mpi3.SeqDatatypeVector[A]): SeqValueBuffer[A] =
      root.bind(dv, offset)

    def @:[A](dt: mpi3.SeqDatatype[A]): SeqValueBuffer[A] = {
      val mpi3.Extent(_, range) = dt.extent
      val mpi3.Extent(_, trueRange) = dt.trueExtent
  ...
  class CommRegionBuffer(parent: Option[(CommRegion, Long)], size: Long)
      extends CommBuffer(
        parent map {
          case (cr, off) =>
            Pointer.pointerToAddress(cr.pointer.getPeer + off, size, mpi3.noRelease)
  ...
  object Bottom extends CommRegion {
    val pointer = mpi3.lib.MPI_BOTTOM
    val root = this
    val offset = 0L
    val size = Long.MaxValue
    val buffer = None
    override def bind[A](
      dv: mpi3.SeqDatatypeVector[A],
      at: Long = 0): SeqValueBuffer[A] = {
      // val size = dv.datatype.trueRange(dv.length)
      // val region =
      //   new CommBuffer(Pointer.pointerToAddress(at, size, mpi3.noRelease), size)
      new BottomValueBuffer(dv.datatype, dv.length)
    }
  }

  def getAddress(ptr: Pointer[_]): mpi3.lib.MPI_Aint =
    if (ptr != null) {
      withOutVar { address: Pointer[mpi3.lib.MPI_Aint] =>
        mpi3.mpiCall(mpi3.lib.MPI_Get_address(ptr, address))
        address(0)
      }
    }
    else 0

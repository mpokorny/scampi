...
trait Errors {
  ...
  def errorClass(errorcode: Int): Int = withOutVar { result: Pointer[Int] =>
    mpi3.mpiCall(mpi3.lib.MPI_Error_class(errorcode, result))
    result(0)
  }

  def errorString(errorcode: Int): String =
    getString(mpi3.lib.MPI_MAX_ERROR_STRING) { (len, buffer) =>
      withOutVar { strlen: Pointer[Int] =>
        mpi3.mpiCall(mpi3.lib.MPI_Error_string(errorcode, buffer, strlen))
      }
    }

  def addErrorClass: Int = withOutVar { result: Pointer[Int] =>
    mpi3.mpiCall(mpi3.lib.MPI_Add_error_class(result))
    result(0)
  }

  def addErrorCode(errorclass: Int): Int = withOutVar { result: Pointer[Int] =>
    mpi3.mpiCall(mpi3.lib.MPI_Add_error_code(errorclass, result))
    result(0)
  }

  def addErrorString(errorcode: Int, string: String) {
    require(
      errorcode > mpi3.lib.MPI_ERR_LASTCODE,
      s"Error string for ${errorcode} cannot be set")
    mpi3.mpiCall(
      mpi3.lib.MPI_Add_error_string(
        errorcode,

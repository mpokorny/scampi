...
trait DerivedDatatypeComponent {
  ...
  abstract class DerivedDatatype[V] extends mpi3.SeqDatatype[V] {

    def free() {
      if (!isNull) {
        mpi3.Datatype.remove(this)
        mpi3.mpiCall(mpi3.lib.MPI_Type_free(handlePtr))
      }
    }
    ...
    private def register() {
      mpi3.mpiCall(dtCreate(handlePtr))
  ...
  // Dup datatypes
  //
  class DupDatatype[V](val basisType: mpi3.SeqDatatype[V])
      extends DerivedDatatype[V] {

    protected def dtCreate(dt: Pointer[mpi3.lib.MPI_Datatype]): Int =
      mpi3.lib.MPI_Type_dup(basisType.handle, dt)
  ...
  class ResizedDatatype[V](
    val basisType: mpi3.SeqDatatype[V],
    newExtent: mpi3.Extent)
      extends DerivedDatatype[V] {

    protected def dtCreate(dt: Pointer[mpi3.lib.MPI_Datatype]): Int =
      mpi3.lib.MPI_Type_create_resized(
        basisType.handle,
  ...
  class ContiguousDatatype[V](
    ...
    protected def dtCreate(dt: Pointer[mpi3.lib.MPI_Datatype]): Int =
      mpi3.lib.MPI_Type_contiguous(length, basisType.handle, dt)
  ...
  //     dt: mpi3.TypedDatatype[V],
  //     cRegion: mpi3.CommRegion,
  //     offset: Long,
  //     length: Int,
  //     linearToBlockIndex: Int => Tuple2[Int, Int])
  //     extends mutable.ArrayBuffer[mpi3.ValueSeq[V]] {
  //   def toBlockedSeq: BlockedSeq[V] =
  //     new BlockedSeq[V](dt, cRegion, offset, this, length, linearToBlockIndex)
  // }

  // class BlockedSeq[V](
  //     val datatype: mpi3.TypedDatatype[V],
  //     val region: mpi3.CommRegion,
  //     val initialOffset: Long,
  //     blocks: mutable.Seq[mpi3.ValueSeq[V]],
  //     val length: Int,
  //     linearToBlockIndex: Int => Tuple2[Int, Int])
  //     extends mutable.IndexedSeq[V] with mpi3.ValueSeq[V] {
  ...
  //     case (blk, off) => {
  //       require(validateBlockIndex(blk, off), mpi3.indexOutOfRangeErrorMsg)
  //       blocks(blk)(off)
  //     }
  //   }
  ...
  class HvectorDatatype[V](
    ...
    protected def dtCreate(dt: Pointer[mpi3.lib.MPI_Datatype]): Int =
      mpi3.lib.MPI_Type_create_hvector(
        numBlocks,
        blocklength,
        stride,
        basisType.handle,
      ...
      val (lengths, displacements) =
        if (blockSeq.length > 0)
          (Pointer.allocateInts(blockSeq.length).as(classOf[Int]),
            mpi3.allocateAint(blockSeq.length))
        else
          (nullPointer[Int], nullPointer[mpi3.lib.MPI_Aint])
      try {
        for (i <- 0 until blockSeq.length) {
          lengths(i) = blockSeq(i).length
          displacements(i) = blockSeq(i).displacement
        }
        mpi3.lib.MPI_Type_create_hindexed(
          blockSeq.length,
          lengths,
          displacements,
          basisType.handle,
    ...
    protected def dtCreate(dt: Pointer[mpi3.lib.MPI_Datatype]): Int = {
      val dspls =
        if (displacements.length > 0)
          ...
          nullPointer[Int]
      try {
        mpi3.lib.MPI_Type_create_indexed_block(
  ...
  //
  // Hindexed block datatypes
  //
  class HindexedBlockDatatype[V](
    val basisType: mpi3.SeqDatatype[V],
    val blocklength: Int,
  ...
  class StructDatatype(blks: StructBlock[_]*)
    ...
    require(blks.forall(_.length >= 0), mpi3.structBlockLengthErrorMsg)

    val blocks = mpi3.StructBlock.withDisplacements(blks)
    ...
    protected def dtCreate(dt: Pointer[mpi3.lib.MPI_Datatype]): Int = {
      ...
      val blocks = mpi3.StructBlock.withDisplacements(blks)
      require(
        mpi3.StructBlock.displacementsAreValid(blocks),
        mpi3.structBlocksAlignmentErrorMsg)
      val lengths = Pointer.allocateInts(blocks.length).as(classOf[Int])
      val addresses = mpi3.allocateAint(blocks.length)
      val datatypes = mpi3.allocateDatatype(blocks.length)
      try {
        ...
        mpi3.lib.MPI_Type_create_struct(
          blocks.length,
          lengths,
          addresses,
          datatypes,
  ...
  //     require(validateSubarrayCoords(c), mpi3.indexOutOfRangeErrorMsg)
  ...
  class SubarrayDatatype[V](
    val basisType: mpi3.SeqDatatype[V],
    val dims: Seq[mpi3.SubarrayDim],
    val order: mpi3.ArrayOrder.ArrayOrder)
      extends DerivedDatatype[V] {
    require(dims.length >= 0, "Number of dimensions must be non-negative")
    require(
      dims.forall(_ match {
        case mpi3.SubarrayDim(size, subsize, start) =>
    ...
    protected def dtCreate(dt: Pointer[mpi3.lib.MPI_Datatype]): Int = {
      ...
      try {
        ...
        mpi3.lib.MPI_Type_create_subarray(
          dims.length,
          sizes,
          subsizes,
          starts,

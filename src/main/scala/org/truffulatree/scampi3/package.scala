...
package object scampi3 {

  protected val properties = {
    val result = new java.util.Properties
    result.load(
      getClass.getClassLoader.getResourceAsStream("scampi3.properties"))
    result
  }

  var libraryName: String = {
    val env = java.lang.System.getenv
    if (env.contains("SCAMPI3_LIBRARY_NAME")) env("SCAMPI3_LIBRARY_NAME")
    else {
      try {
  ...
  lazy val mpi3: Scampi3 with Mpi3LibraryComponent = {
    if (libraryName == "mpich") mpich.mpi3
    else throw new Exception(
      s"Unimplemented MPI3 library interface: ${libraryName}")
  }

...
trait CacheComponent {
  ...
  //   val datatypeVector: mpi2.DatatypeVector[V]

  //   lazy val trueExtent = mpi2.Extent(
  //     datatypeVector.datatype.trueExtent.lowerBound,
  //     datatypeVector.datatype.trueRange(datatypeVector.length))

  //   def toPointer(v: mpi2.ValueBuffer[V]): Pointer[_] = {
  //     require(
  ...
  //   def fromPointer(ptr: Pointer[_]): mpi2.ValueBuffer[V] =
  //     (datatypeVector @:
  //       mpi2.CommBuffer(ptr.offset(-trueExtent.lowerBound), trueExtent.range))
  // }

  class CacheKey[ObjectType,ValueType] protected[scampi2](
    val keyval: Keyval[ObjectType,ValueType],
    initId: Int) {
    def id: Int = initId
  }

  final class UserCacheKey[ObjectType,ValueType] protected[scampi2](
    ...
    def isNull: Boolean = id == mpi2.lib.MPI_KEYVAL_INVALID
    ...
    def free() {
      if (!isNull) mpi2.mpiCall(mpiFreeKeyval(idPtr))
    }
  ...
  trait CommCache extends Cache[mpi2.Comm] {
    type HandleType = mpi2.lib.MPI_Comm

    type CopyFunctionType = mpi2.lib.MPI_Comm_copy_attr_function
    ...
    final def mpiCreateKeyval(
        ...
        keyval: Pointer[Int],
        extraState: Pointer[_]) =
      mpi2.lib.MPI_Comm_create_keyval(copyFn, delFn, keyval, extraState)

    final def mpiFreeKeyval(keyval: Pointer[Int]) =
      mpi2.lib.MPI_Comm_free_keyval(keyval)
  }

  protected val commCacheKeys = mutable.Set[CacheKey[mpi2.Comm,_]](
    mpi2.Comm.TagUBKey,
    mpi2.Comm.UniverseSizeKey,
    mpi2.Comm.LastUsedCodeKey,
    mpi2.Comm.AppNumKey,
    mpi2.Comm.HostKey,
    mpi2.Comm.IOKey)

  trait WinCache extends Cache[mpi2.Win] {
    type HandleType = mpi2.lib.MPI_Win

    type CopyFunctionType = mpi2.lib.MPI_Win_copy_attr_function
    ...
    final def mpiCreateKeyval(
      ...
      keyval: Pointer[Int],
      extraState: Pointer[_]) =
        mpi2.lib.MPI_Win_create_keyval(copyFn, delFn, keyval, extraState)

    final def mpiFreeKeyval(keyval: Pointer[Int]) =
      mpi2.lib.MPI_Win_free_keyval(keyval)
  }

  protected val winCacheKeys = mutable.Set[CacheKey[mpi2.Win,_]](
    mpi2.Win.BaseKey,
    mpi2.Win.SizeKey,
    mpi2.Win.DispUnitKey
  )

  trait DatatypeCache extends Cache[mpi2.Datatype[_]] {
    type HandleType = mpi2.lib.MPI_Datatype

    type CopyFunctionType = mpi2.lib.MPI_Type_copy_attr_function
    ...
    final def mpiCreateKeyval(
      ...
      keyval: Pointer[Int],
      extraState: Pointer[_]) =
      mpi2.lib.MPI_Type_create_keyval(copyFn, delFn, keyval, extraState)

    final def mpiFreeKeyval(keyval: Pointer[Int]) =
      mpi2.lib.MPI_Type_free_keyval(keyval)
  }

  protected val datatypeCacheKeys =
    mutable.Set[CacheKey[mpi2.Datatype[_],_]]()
